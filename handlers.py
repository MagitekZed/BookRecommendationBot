...
def register_handlers(bot):
    ...
    def callback_query(call):
        ...
        if call.data in ['standalone', 'series', 'nopreference']:
            process_format_step(call.message, call.data)
        elif call.data in ['newreleases', 'classics', 'nopreference']:
            process_publicationtime_step(call.message, call.data)
        elif call.data in ['fiction', 'nonfiction']:
    ...
    def process_genre_step(message):
        ...
        keyboard.add(types.InlineKeyboardButton('Stand-Alone', callback_data='standalone'),
                     types.InlineKeyboardButton('Series', callback_data='series'),
                     types.InlineKeyboardButton('No Preference', callback_data='nopreference'))
        # Ask the user if they prefer standalone books or series, and display the keyboard
        bot.send_message(message.chat.id, 'Do you prefer standalone books or series?', reply_markup=keyboard)
    ...
    def process_format_step(message, format):
        ...
        keyboard.add(types.InlineKeyboardButton('New Releases', callback_data='newreleases'),
